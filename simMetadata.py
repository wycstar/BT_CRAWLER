...
def download_metadata(address, infohash, timeout=5):
    ...
    try:
        ...
        tools.storeMetaData(metadata, infohash.encode('hex'))
        '''
        print '---------------------------------------------------------------'
        print 'name:', metadata["name"]
        print 'hash', infohash.encode('hex')
        if 'files' in metadata:
            for x in metadata['files']:
                if 'path.utf-8' in x:
                    print 'path:', '/'.join(x['path.utf-8']), 'length:', x['length']
                else:
                    print 'path:', '/'.join(x['path']), 'length:', x['length']
        else:
